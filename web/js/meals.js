...
export const MEDIAN_MIN_CURVES = 3;

export function medianCurve(curves) {
...
export function drawOverlay(analysis) {
...
export const OVERLAY_NEAREST_MIN = 10;

export function overlayValueAt(curve, minute) {
    let best = null;
    for (const [offset, mgdl] of curve) {
...
/* Засечка на кривой: кольцо цветом панели вокруг точки — тот же приём, что у
   ...
   кривые сходятся. */
export function overlayDot(ctx, minute, mgdl, color) {
    ctx.beginPath();
...
export function drawOverlayCursor(ctx) {
...
export function showOverlayTip(clientX) {
...
export function hideOverlayTip() {
...
export function outcome(meal, targets) {
...
export function recordCell(meal) {
    const cell = document.createElement("td");
...
export function carbsCell(meal) {
...
export function renderMeals(analysis) {
...
export function setFocused(t) {
    state.focusedMeal = t;
    state.previewMeal = t ?? state.hoveredMeal;
    refreshPicked();
}

export function togglePinned(t) {
...
export function renderReviewStats(analysis) {
...
export const RATIO_MIN_MEALS = 3;
...
export function ratioReady(meal) {
    return (
        meal.complete &&
        !meal.cut &&
        !meal.hypo &&
...
export function renderRatio(analysis) {
...
export function ratioRows(dosed, groups, valueOf) {
    const rows = [];
...
export function overlayHoverAt(event) {
...
export function clearOverlayHover() {
