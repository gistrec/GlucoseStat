...
def _lows(readings: list[tuple[datetime, float]], low: int) -> list[dict]:
    """Эпизоды ниже нормы по сырым замерам: когда, как долго, как глубоко.

    Отдельным списком, а не поверх кривой: линия прорежена и о числе эпизодов
    не говорит вовсе — на недельной панели два провала в соседних корзинах
    выглядят одним, а трёхминутный не виден никак. Здесь же считается то, ради
    чего на ночной сахар вообще смотрят: сколько раз и сколько минут.
...
def build_snapshot(
    ...
    for name, (span, step_minutes) in RANGES.items():
        ...
        # Почасовые окна не сравниваются: день против вчерашнего на CGM — в
        # основном шум, стрелка переворачивалась бы почти каждый день, а
        # суточная панель открыта по умолчанию; двое суток против позавчерашних
        # — тот же шум. Тот же довод у карточки GMI. Явная проверка на None
        # обязательна: {**None} бросает TypeError, а месяц без данных —
        # штатный случай.
        if window_stats is None or name in ("day", "two_days"):
